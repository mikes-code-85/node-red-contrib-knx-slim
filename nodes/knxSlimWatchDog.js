...
module.exports = function (RED) {
  function knxSlimWatchDog (config) {
    ...
    // Keep existing flows compatible: before this option was exposed, KNX-Slim
    // node errors were always forwarded by the gateway to every Watchdog node.
    node.listenToKnxSlimNodeErrors = config.listenToKnxSlimNodeErrors !== false && config.listenToKnxSlimNodeErrors !== 'false'
    ...
    node.alreadyNotifiedArray = new Array() // 20/10/2022 array of already notified errors generated by every single KNX-Slim node. This prevents floading with repeated errors, in case of disconnection
    ...
    // This function is called by the knx-slim config node.
    node.watchDogTimerReset = () => {
    ...
    // 16/02/2020 This function is called by the knx-slim config node.
    node.signalNodeErrorCalledByConfigNode = _oError => {
      if (!node.listenToKnxSlimNodeErrors) return

      // Report an error from knx-slim node.
      // let oError = {nodeid:node.id,topic:node.outputtopic,devicename:devicename,GA:GA,text:text};
      const msg = {
        type: 'NodeError',
        checkPerformed: 'Self KNX-Slim node reporting a red color status',
    ...
    if (node.serverKNX) {
      ...
      if (hasHealthCheckTarget || node.listenToKnxSlimNodeErrors) {
        node.serverKNX.addClient(node)
        if (node.autoStart && hasHealthCheckTarget) node.StartWatchDogTimer() // Autostart watchdog
      }
    }
  }
  RED.nodes.registerType('knxSlimWatchDog', knxSlimWatchDog)
}
